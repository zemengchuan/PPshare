...
def macro_cons_gold():
    ...
    """
    temp_df = get_from_jin10_4(1)
    return temp_df


def macro_cons_silver():
    """
    全球最大白银 ETF—SPDR Gold Trust 持仓报告, 数据区间从 20041118-至今
    :return: 持仓报告
    :rtype: pandas.DataFrame
    """
    temp_df = get_from_jin10_4(2)
    return temp_df
...
def macro_cons_opec_month():
    ...
    t = time.time()
    big_df = pd.DataFrame()
    headers = cs.HEADERS['5jin10']
    url = cs.URL_TYPE['https']+cs.URL_DIC['data-jin10']+cs.URL_PARA['cons_opec_month_1']
    res = requests.get(
        url.format(str(int(round(t * 1000)))),
        headers=headers,
    )  # 日期序列
    ...
    for item in bar:
        ...
        url = cs.URL_TYPE['https']+cs.URL_DIC['data-jin10']+cs.URL_PARA['cons_opec_month_2']
        res = requests.get(
            url.format(item,str(int(round(t * 1000)))),
            headers=headers,
        )
